...
\begin{myalign*}
    y_n &\equiv f(\* x_n) := \beta_0 + \beta_1 x_{n1} + ... = \beta_0 + \* x^T_n \bm \beta
\end{myalign*}
...
\begin{itemize}
    ...
    \begin{enumerate}
        \item \textit{necessary}: gradient equal zero: $\frac{d \L(\bm \beta^*)}{d \bm \beta} = 0$
...
\begin{itemize}
	...
	\end{align}
	\item Complexity of computing $\bm \beta$: (1) $O(D^2 N + D^3)$, (2) $O(D N^2 + N^3)$
	\item Thus we have
	\begin{myalign*}
...
\begin{itemize}
	...
	\end{myalign*}
	\item Minimum doesn't change with a rescaling of $\bm \beta$
...
\begin{itemize}
    ...
    \end{myalign*}
    \item \textbf{Backpropagation} is a technic to compute the gradient in time linear in the number of training points and the number of weights and is nothing else than a chain rule of differential calculus.
    ...
    \begin{enumerate}
    	...
    	\begin{myalign*}
    	    \bm \delta^{(k - 1)}_n = diag[(\*h' (a_n^{(k)})] (\*B^{(k)})^T \bm \delta_n^{(k)}
    	\end{myalign*}
